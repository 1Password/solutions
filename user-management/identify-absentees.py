...
def parse_user_list_output() -> List[Dict[str, str]]:
    ...
    try:
        result = subprocess.run(
            ["op", "user", "list"], capture_output=True, text=True, check=True
        )
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        if TQDM_AVAILABLE:
            ...
            for i, result in enumerate(
                executor.map(fetch_user_details, active_users), 1
            ):
...
def print_boxed_summary(
    days: int, total_users: int, active_users: int, error: Optional[str] = None
) -> None:
    ...
    print(
        f"│ {total_line.ljust(max_length)} │"
        if not error
        else f"│ {total_line.ljust(max_length)} │"
    )
...
def print_idle_users_table(idle_users: List[Dict[str, Any]]) -> None:
    ...
    days_max = (
        max(max(len(str(u.get("days_idle", ""))) for u in idle_users), len(days_header))
        + 2
    )
    login_max = (
        max(max(len(u.get("last_auth_at", "")) for u in idle_users), len(login_header))
        + 2
    )

    # Build the table borders
    top_border = (
        "┌" + ("─" * email_max) + "┬" + ("─" * days_max) + "┬" + ("─" * login_max) + "┐"
    )
...
def main() -> None:
    ...
    active_users.sort(
        key=lambda x: (
            float("inf")
            if x.get("last_auth_at") in ["0001-01-01T00:00:00Z", "01/01/0001 00:00:00"]
            else (
                float("inf")
                if not x.get("last_auth_at")
                else (
                    (now - parse_date(x["last_auth_at"])).total_seconds() / 86400
    ...
    for user in active_users:
        ...
        if not last_login:
            print(
                f"  {user['email'].ljust(50)} Weird date format ('{last_auth_at}'), skipping."
            )
            continue
        ...
        color = (
            "\033[31m" if diff_days > days else "\033[32m"
        )  # Red for idle, green for active
        reset = "\033[0m"
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        for user in idle_users:
            writer.writerow(
                [user["id"], user["name"], user["email"], user.get("last_auth_at", "")]
            )

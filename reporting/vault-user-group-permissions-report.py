...
def writeReport(vaults: Vault):
    ...
    with open(f"{outputPath}/vaultAccessReport.csv", "w", newline="") as outputFile:
        ...
        fields = ["vaultName", "vaultUUID", "userName",
                  "userEmail", "userUUID", "groupName", "groupUUID", "permissions"]
        ...
        for vault in vaults:
            ...
            for user in vault.users:
                if user['assignment'] == "Direct" and user['state'] == "ACTIVE":
                    csvWriter.writerow(
                        [vaultName, vaultUUID, user['name'], user['email'], user['uuid'], None, None, user['permissions']])
            for group in vault.groups:
                csvWriter.writerow(
                    [vaultName, vaultUUID, None, None, None, group['name'], group['uuid'], group['permissions']])
...
def main():
    ...
    for vault in vaults[:8]:
        ...
        for user in users:
            vault.users.append(
                {'name': user['name'], 'email': user['email'], 'uuid': user['id'], 'assignment': 'Direct', 'state': user['state'], 'permissions': user['permissions']})
    ...
    # For assigned groups, decompose into individual users
        ...
        for group in groups:
            vault.groups.append(
                {'name': group['name'], 'uuid': group['id'], 'permissions': group['permissions']})
            try:
    ...
    try:
        ...
        sys.exit("Unable to write data to a file on disk.", e)
